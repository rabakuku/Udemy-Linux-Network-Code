...
# Labs 1–5: Libreswan
# Labs 6–10: StrongSwan
# Labs 11–12: WireGuard + IPsec (transport)
# - Config labs (1,2,6,7,11): apply prints steps (does NOT change system)
# - Troubleshooting labs (3,4,5,8,9,10,12): apply writes broken configs & toggles firewall
...
1. Scenario One
2. Scenario Two (Libreswan RSA/cert)
3. Troubleshooting 1
4. Troubleshooting 2
5. Troubleshooting 3
6. Scenario Six (StrongSwan PSK)
7. Scenario Seven (StrongSwan RSA/cert)
8. Troubleshooting 4 (StrongSwan)
9. Troubleshooting 5 (StrongSwan)
10. Troubleshooting 6 (StrongSwan)
11. Scenario Eleven (WireGuard + IPsec transport)
12. Troubleshooting 7 (WireGuard + IPsec transport)
Usage:
...
# --- Lab 2: Libreswan RSA/cert (no changes applied) ---
lab2_apply() {
  echo "Lab 2 (Config): Site-to-site IPsec with IKEv2 RSA/cert authentication (no changes applied)."
  print_netplan_yaml_server1
  print_netplan_yaml_server2
  cat <<EOF
...
- Create a CA and host certificates, import into Libreswan's NSS DB.
- Set authby=rsasig; leftrsasigkey/right rsasigkey as %cert.
- Open UDP/500 & UDP/4500; auto=start.
EOF
}

# --- Lab 3: Libreswan Troubleshooting (wrong ID + weak crypto) ---
lab3_apply() {
...
# --- Lab 4: Libreswan Troubleshooting (RSA without certs + blocked ports) ---
lab4_apply() {
...
# --- Lab 5: Libreswan Troubleshooting (wrong loopbacks + legacy crypto) ---
lab5_apply() {
...
# --- Lab 6: StrongSwan PSK (no changes applied) ---
lab6_apply() {
...
# --- Lab 7: StrongSwan RSA/cert (no changes applied) ---
lab7_apply() {
...
# --- Lab 8: StrongSwan Troubleshooting (RSA without certs + weak crypto) ---
lab8_apply() {
...
# --- Lab 9: StrongSwan Troubleshooting (wrong IDs + wrong loopbacks) ---
lab9_apply() {
...
# --- Lab 10: StrongSwan Troubleshooting (weak crypto + ICMP blocked) ---
lab10_apply() {
...
# --- Lab 11: WireGuard + IPsec transport (Config, no changes applied) ---
lab11_apply() {
  echo "Lab 11 (Config): WireGuard tunnel with IPsec transport (no changes applied)."
  print_netplan_yaml_server1
  print_netplan_yaml_server2
  cat <<EOF
...
# --- Lab 12: WireGuard + IPsec transport (Troubleshooting: wrong peer key + wrong PSK + blocked 51820) ---
lab12_apply() {
  echo "Lab 12 (Troubleshooting): Broken WireGuard + IPsec config (wrong peer key, wrong PSK, UDP/51820 blocked)."
  q apt-get update -y
...
lab2_check() {
  begin_check
  # Students configure RSA/cert; basic reachability
  can_ping "$S2_Dummy" && good "peer loopback (${S2_Dummy}) reachable" || miss "Cannot ping peer loopback"
  end_check
}
...
# --- Lab 12 check (WireGuard + IPsec troubleshooting) ---
lab12_check() {
  begin_check
...
print_solution() {
  ...
  case "$lab" in
    ...
    2)
      cat <<'EOS'
# =========================
# Lab 2: IPsec with RSA/Cert (Libreswan)
# =========================

# ---- 1. Netplan (both servers) ----
# See YAML above for server1/server2, then run:
# sudo netplan apply

# ---- 2. Install Libreswan and NSS tools ----
sudo apt-get update -y
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y libreswan libnss3-tools ufw
...
ipsec pki --self --ca --lifetime 3650 --in ca-key.pem --type rsa \
  --dn "CN=IPsec CA" --outform pem > ca-cert.pem

# ---- 4. Generate host keys + issue certs ----
# Server1:
ipsec pki --gen --type rsa --size 2048 --outform pem > server1-key.pem
ipsec pki --pub --in server1-key.pem --type rsa | \
  ipsec pki --issue --lifetime 1825 \
    --cacert ca-cert.pem --cakey ca-key.pem \
    --dn "CN=server1" --san "server1" --flag serverAuth --outform pem > server1-cert.pem

# Server2:
ipsec pki --gen --type rsa --size 2048 --outform pem > server2-key.pem
ipsec pki --pub --in server2-key.pem --type rsa | \
  ipsec pki --issue --lifetime 1825 \
    --cacert ca-cert.pem --cakey ca-key.pem \
    --dn "CN=server2" --san "server2" --flag serverAuth --outform pem > server2-cert.pem

# ---- 5. Prepare NSS DB on each server ----
sudo ipsec initnss

# ---- 6. Import CA and host certs into NSS DB ----
# On Server1:
sudo certutil -A -n "IPsec CA" -t "CT,," -d sql:/etc/ipsec.d -a -i ca-cert.pem
sudo certutil -A -n "server1" -t "u,u,u" -d sql:/etc/ipsec.d -a -i server1-cert.pem
# Import private key using a PKCS#12 (empty pass)
openssl pkcs12 -export -inkey server1-key.pem -in server1-cert.pem -name server1 -passout pass: | \
  sudo pk12util -i - -d sql:/etc/ipsec.d -W ""

# On Server2:
sudo certutil -A -n "IPsec CA" -t "CT,," -d sql:/etc/ipsec.d -a -i ca-cert.pem
sudo certutil -A -n "server2" -t "u,u,u" -d sql:/etc/ipsec.d -a -i server2-cert.pem
openssl pkcs12 -export -inkey server2-key.pem -in server2-cert.pem -name server2 -passout pass: | \
  sudo pk12util -i - -d sql:/etc/ipsec.d -W ""

# ---- 7. Place certs/keys appropriately ----
# Copy ca-cert.pem to both servers (if you created them on a separate machine).
# Ensure 'ipsec' can access the NSS DB (we used sql:/etc/ipsec.d) and cert nicknames.

# ---- 8. Configure /etc/ipsec.d/section6.conf on both servers ----
# Server1:
sudo tee /etc/ipsec.d/section6.conf >/dev/null <<'CONF1'
conn s6-lab2
  type=tunnel
  keyexchange=ikev2
  left=10.10.10.1
...
# Server2:
sudo tee /etc/ipsec.d/section6.conf >/dev/null <<'CONF2'
conn s6-lab2
  type=tunnel
  keyexchange=ikev2
  left=10.10.10.2
...
# ---- 9. No /etc/ipsec.secrets needed for RSA/cert ----
...
ipsec whack --status
journalctl -u ipsec -b --no-pager
# If IDs mismatch, ensure leftid/rightid match the cert CN or SAN (e.g., @server1/@server2).
EOS
      ;;
...
EOS
    ...
    12)
      cat <<'EOS'
# Troubleshooting 7 (WireGuard + IPsec transport)
# Fix sequence:
...
interactive_menu() {
  while true; do
    ...
    case "$opt" in
      1) read -rp "Lab (1-12): " lab; apply_lab "$lab"; read -rp "Press Enter..." ;;
      2) read -rp "Lab (1-12): " lab; do_check "$lab"; read -rp "Press Enter..." ;;
      3) reset_all; read -rp "Press Enter..." ;;
      4) status; read -rp "Press Enter..." ;;
      5) print_list; read -rp "Press Enter..." ;;
      6) read -rp "Lab (1-12): " lab; print_solution "$lab"; read -rp "Press Enter..." ;;
      q|Q) exit 0 ;;
