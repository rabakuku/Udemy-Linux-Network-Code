...
# Optional peer pings
PING_PEER="${S3_PING:-1}"; PING_COUNT="${S3_PING_COUNT:-2}"; PING_TIMEOUT="${S3_PING_TIMEOUT:-1}"

# ===== Paths / State =====
LAB_ROOT="/etc/labs-menu"; STATE_FILE="${LAB_ROOT}/state"
NETPLAN_DIR="/etc/netplan"
S3_NETPLAN_FILE="${NETPLAN_DIR}/10-section3-nm.yaml"
NM_CONF_DIR="/etc/NetworkManager/conf.d"
NM_CONN_V10="s3-vlan10"; NM_CONN_V20="s3-vlan20"; NM_CONN_ACC="s3-access"
...
save_state(){ local k="$1" v="$2"; mkdirs; touch "$STATE_FILE"; grep -q "^${k}=" "$STATE_FILE" 2>/dev/null && sed -i "s/^${k}=.*/${k}=${v}/" "$STATE_FILE" || echo "${k}=${v}" >>"$STATE_FILE"; }
get_state(){ local k="$1"; [[ -f "$STATE_FILE" ]] || { echo ""; return 0; }; grep "^${k}=" "$STATE_FILE" | tail -n1 | cut -d= -f2- || true; }
need_root(){ [[ $EUID -ne 0 ]] && { echo -e "${WARN} Please run as root: sudo $0 $*"; exit 1; }; }
primary_if(){
  if ip link show ens4 >/dev/null 2>&1; then echo "ens4"; return 0; fi
  if ip link show lan0 >/dev/null 2>&1; then echo "lan0"; return 0; fi
  ip -o link show | awk -F': ' '{print $2}' | grep -E '^(en|eth|eno|ens|enp|lan)[a-z0-9]+' | grep -v '^lo$' | head -n1
}
...
nm_require_and_ensure_renderer(){
  # Install & start NM
  if ! command -v nmcli >/dev/null 2>&1; then sudo apt-get update -y; sudo apt-get install -y network-manager; fi
  q systemctl enable --now NetworkManager
  if ! nm_is_active; then echo -e "${FAIL} NetworkManager is not active. Start it: sudo systemctl enable --now NetworkManager"; exit 2; fi
...
YAML
  q netplan generate; q netplan apply  # Netplan drives who manages the NICs. [1](https://documentation.ubuntu.com/core/explanation/system-snaps/network-manager/how-to-guides/networkmanager-and-netplan/)[2](https://www.cyberciti.biz/faq/change-netplan-renderer-from-networkd-to-networkmanager/)

  # NM conf.d overrides (manage everything; allow ifupdown-managed)
  mkdir -p "${NM_CONF_DIR}"
...
managed=true
INI
  q systemctl restart NetworkManager  # conf.d precedence & overrides take effect. [3](https://www.networkmanager.dev/docs/api/latest/NetworkManager.conf.html)[5](https://unix.stackexchange.com/questions/665612/how-do-i-set-a-network-device-to-be-managed)[4](https://askubuntu.com/questions/71159/network-manager-says-device-not-managed)
  sleep 1
}

# --- NM device helpers
nm_conn_device(){ local name="$1" dev=""; dev="$(nmcli -t -f NAME,DEVICE connection show --active 2>/dev/null | awk -F: -v n="$name" '$1==n{print $2;exit}')"; [[ -z "$dev" ]] && dev="$(nmcli -t -g connection.interface-name connection show "$name" 2>/dev/null || true)"; [[ -z "$dev" ]] && dev="$(primary_if)"; echo "$dev"; }
peer_ip(){ local role="$1" s1="$2" s2="$3"; [[ "$role" == "1" ]] && echo "$s2" || echo "$s1"; }
ping_on_iface(){ local ifc="$1" peer="$2" lbl="$3"; if [[ "${PING_PEER}" == "1" ]] && command -v ping >/dev/null 2>&1; then ping -I "$ifc" -c "${PING_COUNT}" -W "${PING_TIMEOUT}" "$peer" >/dev/null 2>&1 && echo -e "${OK} Peer reachable (${lbl} -> ${peer})" || echo -e "${WARN} Peer not reachable (${lbl} -> ${peer}); check mode/subnet/cable/VLAN tag)"; else echo -e "${INFO} Skipping ping (${lbl}); enable with S3_PING=1"; fi; }

# --- Check reporting
good(){ echo -e "${OK} $*"; } ; miss(){ echo -e "${FAIL} $*"; FAILS=$((FAILS+1)); } ; begin_check(){ FAILS=0; } ; end_check(){ [[ ${FAILS:-0} -eq 0 ]] && echo -e "${OK} All checks passed" || { echo -e "${FAIL} ${FAILS} issue(s) found"; exit 4; }; }

# ===== Summaries =====
summary_for_lab(){ local lab="$1"; case "$lab" in
  1) cat <<EOF
...
# APPLY FUNCTIONS
# =========================
lab1_apply(){ nm_require_and_ensure_renderer; local role ip ifc; role="$(get_state role)"; [[ -z "$role" ]] && role="1"; ip="$V10_S1_IP"; [[ "$role" == "2" ]] && ip="$V10_S2_IP"; ifc="$(primary_if)"; [[ -z "$ifc" ]] && { echo -e "${FAIL} No primary NIC detected"; exit 2; }
  q nmcli con delete "${NM_CONN_V10}"
  nmcli con add type vlan ifname vlan10 con-name "${NM_CONN_V10}" dev "${ifc}" id 10 ipv4.addresses "${ip}/${PREFIX}" ipv4.method manual ipv6.method ignore >/dev/null
  q nmcli con up "${NM_CONN_V10}"
}
lab2_apply(){ nm_require_and_ensure_renderer; local role ip ifc; role="$(get_state role)"; [[ -z "$role" ]] && role="1"; ip="$V20_S1_IP"; [[ "$role" == "2" ]] && ip="$V20_S2_IP"; ifc="$(primary_if)"; [[ -z "$ifc" ]] && { echo -e "${FAIL} No primary NIC detected"; exit 2; }
  q nmcli con delete "${NM_CONN_V20}"
  nmcli con add type vlan ifname vlan20 con-name "${NM_CONN_V20}" dev "${ifc}" id 20 ipv4.addresses "${ip}/${PREFIX}" ipv4.method manual ipv6.method ignore >/dev/null
...
lab5_apply(){ nm_require_and_ensure_renderer; local role ip ifc; role="$(get_state role)"; [[ -z "$role" ]] && role="1"; ip="$ACC_S1_IP"; [[ "$role" == "2" ]] && ip="$ACC_S2_IP"; ifc="$(primary_if)"; [[ -z "$ifc" ]] && { echo -e "${FAIL} No primary NIC detected"; exit 2; }
  q nmcli con delete "${NM_CONN_ACC}"
  nmcli con add type ethernet ifname "${ifc}" con-name "${NM_CONN_ACC}" ipv4.addresses "${ip}/${PREFIX}" ipv4.method manual ipv6.method ignore >/dev/null
  q nmcli con down "${NM_CONN_V10}"; q nmcli con down "${NM_CONN_V20}"
  q nmcli con up "${NM_CONN_ACC}"
}

# =========================
# CHECK FUNCTIONS (unchanged logic + pings)
# =========================
lab1_check(){ begin_check; nm_is_active && good "NetworkManager is active" || miss "NetworkManager is not active"
  nmcli -t -f NAME connection show | grep -qx "${NM_CONN_V10}" && good "Connection '${NM_CONN_V10}' exists" || miss "Connection '${NM_CONN_V10}' missing"
  local role ip; role="$(get_state role)"; [[ -z "$role" ]] && role="1"; ip="$V10_S1_IP"; [[ "$role" == "2" ]] && ip="$V10_S2_IP"
...
lab2_check(){ begin_check; nm_is_active && good "NetworkManager is active" || miss "NetworkManager is not active"
  nmcli -t -f NAME connection show | grep -qx "${NM_CONN_V20}" && good "Connection '${NM_CONN_V20}' exists" || miss "Connection '${NM_CONN_V20}' missing"
  local role ip; role="$(get_state role)"; [[ -z "$role" ]] && role="1"; ip="$V20_S1_IP"; [[ "$role" == "2" ]] && ip="$V20_S2_IP"
...
lab3_check(){ begin_check; nm_is_active && good "NetworkManager is active" || miss "NetworkManager is not active"
  local role ip10 ip20; role="$(get_state role)"; [[ -z "$role" ]] && role="1"; ip10="$V10_S1_IP"; [[ "$role" == "2" ]] && ip10="$V10_S2_IP"; ip20="$V20_S1_IP"; [[ "$role" == "2" ]] && ip20="$V20_S2_IP"
...
lab4_check(){ begin_check
  local role ip10 ip20 peer10 peer20; role="$(get_state role)"; [[ -z "$role" ]] && role="1"
  ip10="$V10_S1_IP"; [[ "$role" == "2" ]] && ip10="$V10_S2_IP"; ip20="$V20_S1_IP"; [[ "$role" == "2" ]] && ip20="$V20_S2_IP"
  peer10="$(peer_ip "$role" "$V10_S1_IP" "$V10_S2_IP")"; peer20="$(peer_ip "$role" "$V20_S1_IP" "$V20_S2_IP")"
  nmcli -t -f NAME connection show | grep -qx "${NM_CONN_V10}" && nmcli -t -f NAME connection show | grep -qx "${NM_CONN_V20}" && good "Both VLAN connections exist" || miss "Trunk incomplete: one or both VLAN connections missing"
  ip link show vlan10 >/dev/null 2>&1 && good "vlan10 present" || miss "vlan10 missing"
  ip -d link show vlan10 | grep -q "vlan id 10" && good "vlan10 id 10 OK" || miss "vlan10 id mismatch"
  ip -4 -o addr show dev vlan10 | grep -q " ${ip10}/" && good "vlan10 has ${ip10}/${PREFIX}" || miss "vlan10 missing ${ip10}/${PREFIX}"
  ip link show vlan20 >/dev/null 2>&1 && good "vlan20 present" || miss "vlan20 missing"
  ip -d link show vlan20 | grep -q "vlan id 20" && good "vlan20 id 20 OK" || miss "vlan20 id mismatch"
  ip -4 -o addr show dev vlan20 | grep -q " ${ip20}/" && good "vlan20 has ${ip20}/${PREFIX}" || miss "vlan20 missing ${ip20}/${PREFIX}"
  ping_on_iface "vlan10" "$peer10" "TRUNK-vlan10"; ping_on_iface "vlan20" "$peer20" "TRUNK-vlan20"; end_check
}
lab5_check(){ begin_check
  local role ip ifc peer; role="$(get_state role)"; [[ -z "$role" ]] && role="1"
  ip="$ACC_S1_IP"; [[ "$role" == "2" ]] && ip="$ACC_S2_IP"; peer="$(peer_ip "$role" "$ACC_S1_IP" "$ACC_S2_IP")"
  ifc="$(primary_if)"
  nmcli -t -f NAME connection show | grep -qx "${NM_CONN_ACC}" && good "Access connection '${NM_CONN_ACC}' exists" || miss "Access connection '${NM_CONN_ACC}' missing"
  [[ -n "$ifc" ]] && ip -4 -o addr show dev "$ifc" | grep -q " ${ip}/" && good "Base ${ifc} has ${ip}/${PREFIX}" || miss "Base ${ifc} lacks ${ip}/${PREFIX}"
  ping_on_iface "$ifc" "$peer" "ACCESS-untagged"
  if ip link show vlan10 >/dev/null 2>&1 || ip link show vlan20 >/dev/null 2>&1; then echo -e "${WARN} VLAN subinterfaces present in access mode; ensure peer matches."; fi
  end_check
}
lab6_check(){ begin_check
  local active_conns mode=""; active_conns="$(nmcli -t -f NAME connection show --active 2>/dev/null || true)"
  if echo "$active_conns" | grep -qx "${NM_CONN_ACC}"; then mode="access"; elif echo "$active_conns" | grep -qE "^(${NM_CONN_V10}|${NM_CONN_V20})$"; then mode="trunk"; else mode="unknown"; fi
  ...
  local role ifc ip10 ip20 accip peer10 peer20 peeracc; role="$(get_state role)"; [[ -z "$role" ]] && role="1"; ifc="$(primary_if)"
  ip10="$V10_S1_IP"; [[ "$role" == "2" ]] && ip10="$V10_S2_IP"; ip20="$V20_S1_IP"; [[ "$role" == "2" ]] && ip20="$V20_S2_IP"; accip="$ACC_S1_IP"; [[ "$role" == "2" ]] && accip="$ACC_S2_IP"
  peer10="$(peer_ip "$role" "$V10_S1_IP" "$V10_S2_IP")"; peer20="$(peer_ip "$role" "$V20_S1_IP" "$V20_S2_IP")"; peeracc="$(peer_ip "$role" "$ACC_S1_IP" "$ACC_S2_IP")"
  if [[ "$mode" == "trunk" ]]; then
    ip link show vlan10 >/dev/null 2>&1 && good "vlan10 present" || miss "vlan10 missing"
    ...
    ip -4 -o addr show dev vlan20 | grep -q " ${ip20}/" && good "vlan20 has ${ip20}/${PREFIX}" || miss "vlan20 missing ${ip20}/${PREFIX}"
    ping_on_iface "vlan10" "$peer10" "TRUNK-vlan10"; ping_on_iface "vlan20" "$peer20" "TRUNK-vlan20"
    [[ -n "$ifc" ]] && ip -4 -o addr show dev "$ifc" | grep -q " ${accip}/" && echo -e "${WARN} Base ${ifc} has untagged ${accip}/${PREFIX} while in trunk (peer may be access)."
  elif [[ "$mode" == "access" ]]; then
    [[ -n "$ifc" ]] && ip -4 -o addr show dev "$ifc" | grep -q " ${accip}/" && good "Base ${ifc} has ${accip}/${PREFIX}" || miss "Base ${ifc} lacks ${accip}/${PREFIX}"
    ping_on_iface "$ifc" "$peeracc" "ACCESS-untagged"
    if ip link show vlan10 >/dev/null 2>&1 || ip link show vlan20 >/dev/null 2>&1; then echo -e "${WARN} VLANs present in access mode; ensure peer matches."; fi
  fi; end_check
}
...
# =========================
apply_lab(){ local lab="$1" role="$2"; save_state role "$role"; summary_for_lab "$lab"; case "$lab" in
  1) lab1_apply ;; 2) lab2_apply ;; 3) lab3_apply ;; 4) lab4_apply ;; 5) lab5_apply ;; 6) nm_require_and_ensure_renderer ;;
  *) echo -e "${FAIL} Unknown lab $lab"; exit 2 ;; esac; save_state lab "$lab"; echo -e "${OK} Applied Lab ${lab}"; }
do_check(){ local lab="$1"; case "$lab" in 1) lab1_check ;; 2) lab2_check ;; 3) lab3_check ;; 4) lab4_check ;; 5) lab5_check ;; 6) lab6_check ;; *) echo -e "${FAIL} Unknown lab $lab"; exit 2 ;; esac; }

# =========================
# RESET / STATUS / LIST
# =========================
reset_all(){ nm_is_active || { q systemctl enable --now NetworkManager; }; q nmcli con delete "${NM_CONN_V10}"; q nmcli con delete "${NM_CONN_V20}"; q nmcli con delete "${NM_CONN_ACC}"
  : > "${STATE_FILE}" 2>/dev/null || true
  echo -e "${OK} Reset complete"
}
status(){ local lab; lab="$(get_state lab || true)"; [[ -z "$lab" ]] && lab="(none)"; echo -e "${INFO} Current Lab: ${lab}"; echo -e "${INFO} Links:"; ip -o link show | awk -F': ' '{print " - " $2}'
  local ifc; ifc="$(primary_if)"; [[ -n "$ifc" ]] && { echo -e "${INFO} Primary: ${ifc}"; ip -4 -o addr show dev "${ifc}" || true; }
  if command -v nmcli >/dev/null 2>&1; then echo -e "${INFO} NM devices:"; nmcli -t -f DEVICE,STATE,CONNECTION device status || true; echo -e "${INFO} NM active connections:"; nmcli -t -f NAME,DEVICE connection show --active || true; fi
}
print_list(){ cat <<EOF
...
# =========================
# SOLUTIONS / TIPS (unchanged texts)
# =========================
print_solution(){ local lab="$1"; echo -e "${BOLD}Solution for Lab ${lab}${NC}"; echo "----------------------------------------"; case "$lab" in
  1) cat <<'EOS'
...
2) Validate per mode:
   TRUNK: both VLANs present, correct IDs, correct IPs, peer pings ok
   ACCESS: base NIC has untagged role IP, peer pings ok
3) Common issues: mode mismatch, duplicate IPs (ARP), wrong VLAN IDs, carrier down
EOS
  ;;
esac; echo "----------------------------------------"; }
print_tip(){ local lab="$1"; echo -e "${BOLD}Tips for Lab ${lab}${NC}"; echo "----------------------------------------"; case "$lab" in
...
# =========================
interactive_menu(){ while true; do clear; echo -e "${BOLD}Section 3 Labs Menu${NC}"
  echo "1) Apply Lab"; echo "2) Check Lab"; echo "3) Reset"; echo "4) Status"; echo "5) List Labs"; echo "6) Solutions"; echo "7) Tips"; echo "q) Quit"
  ...
  case "$opt" in
    ...
    q|Q) exit 0 ;; *) echo "Invalid selection"; sleep 1 ;;
  esac; done; }
main(){ need_root; mkdirs; [[ $# -lt 1 ]] && { interactive_menu; exit 0; }
  case "$1" in
    list) print_list; exit 0 ;; status) status; exit 0 ;; reset) reset_all; exit 0 ;;
  ...
  local lab="$1"; [[ $# -lt 2 ]] && { echo -e "${FAIL} Usage: $0 <lab#> apply|check [role]"; exit 2; }
  case "$2" in
